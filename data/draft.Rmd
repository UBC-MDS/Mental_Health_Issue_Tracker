...
(pie <- df %>%
   ...
   summarise(count=n())%>%
   ggplot(aes(x="", y=count, fill=wrk_interference_when_treated))+
...
(pie2 <- df %>%
   group_by(wrk_interference_No_treatement)%>%
   summarise(count=n())%>%
   ggplot(aes(x="", y=count, fill=wrk_interference_No_treatement))+
   geom_bar(width = 1, stat = "identity")+
   coord_polar("y", start=0)+
